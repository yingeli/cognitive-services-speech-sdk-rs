...
use crate::ffi::{
    speech_config_get_property_bag, speech_config_release, speech_config_set_audio_output_format,
    speech_config_set_profanity, speech_config_set_service_property,
    ...
    speech_translation_config_from_host, speech_translation_config_from_subscription, SmartHandle,
    SPXHANDLE, SPXPROPERTYBAGHANDLE, SPXSPEECHCONFIGHANDLE,
};
// use crate::speech::EmbeddedSpeechConfig;
use std::ffi::CString;
use std::mem::MaybeUninit;

/// SpeechTranslationConfig is the class that defines configurations for speech translation recognition, or speech translation synthesis.
#[derive(Debug)]
...
impl SpeechTranslationConfig {
    /// Creates a SpeechTranslationConfig instance from a valid handle. This is for internal use only.
    ///
    /// # Safety
    /// `handle` must be a valid handle to a live speech translation config.
    pub unsafe fn from_handle(handle: SPXHANDLE) -> Result<SpeechTranslationConfig> {
        unsafe {
            ...
            let result = SpeechTranslationConfig {
    ...
    pub fn from_subscription<S>(subscription: S, region: S) -> Result<SpeechTranslationConfig>
    where
        S: Into<Vec<u8>>,
    {
    ...
    /// Creates an instance of the speech translation config with specified authorization token and
    /// region.
    ...
    pub fn from_auth_token<S>(auth_token: S, region: S) -> Result<SpeechTranslationConfig>
    where
        S: Into<Vec<u8>>,
    {
        ...
        S: Into<Vec<u8>>,
    {
        ...
        unsafe {
            ...
            convert_err(
                ret,
                "SpeechTranslationConfig::from_endpoint_with_subscription error",
            )?;
            SpeechTranslationConfig::from_handle(handle.assume_init())
        }
    }
    ...
    /// "de-DE".
    /// Only the parameters that are not specified in the endpoint URI can be set by other APIs.
    /// Note: If the endpoint requires a subscription key for authentication, use NewSpeechTranslationConfigFromEndpointWithSubscription
    ...
    pub fn from_endpoint<S>(endpoint: S) -> Result<SpeechTranslationConfig>
    where
        S: Into<Vec<u8>>,
    {
        let c_endpoint = CString::new(endpoint)?;
    ...
    /// Note: To use an authorization token with host, use NewSpeechTranslationConfigFromHost,
    /// and then call SetAuthorizationToken() on the created SpeechTranslationConfig instance.
        ...
        S: Into<Vec<u8>>,
    {
        ...
        unsafe {
            ...
            convert_err(
                ret,
                "SpeechTranslationConfig::from_host_with_subscription error",
            )?;
            SpeechTranslationConfig::from_handle(handle.assume_init())
        }
    }

    /// Creates an instance of SpeechTranslationConfig with specified host.
    /// This method is intended only for users who use a non-default service host. Standard resource path will be assumed.
    /// For services with a non-standard resource path or no path at all, use FromEndpoint instead.
    /// Note: Query parameters are not allowed in the host URI and must be set by other APIs.
    /// Note: If the host requires a subscription key for authentication, use NewSpeechTranslationConfigFromHostWithSubscription to pass
    ...
    /// call SetAuthorizationToken() on the created SpeechTranslationConfig instance.
    pub fn from_host<S>(host: S) -> Result<SpeechTranslationConfig>
    where
        S: Into<Vec<u8>>,
    {
        let c_host = CString::new(host)?;
    ...
    pub fn set_service_property(
        ...
        unsafe {
            ...
            convert_err(ret, "SpeechTranslationConfig.set_service_property error")?;
            Ok(())
        }
    }
    ...
    pub fn set_profanity_option(&mut self, profanity_option: ProfanityOption) -> Result<()> {
        unsafe {
            ...
            convert_err(ret, "SpeechTranslationConfig.set_profanity_option error")?;
            Ok(())
        }
    }
    ...
    pub fn set_speech_synthesis_output_format(
        ...
        convert_err(
            ret,
            "SpeechTranslationConfig.set_speech_synthesis_output_format error",
        )?;
        Ok(())
    }
    ...
    /// Region key that used to create Speech Translation Recognizer.
    ...
    /// Gets input language to the speech translation recognition.
    ...
    // Sets the speech translation synthesis output format (e.g. Riff16Khz16BitMonoPcm).
